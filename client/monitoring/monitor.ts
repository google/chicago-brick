...
let handleModelChange = (change: any) => {
  if (change.layout) {
...
let handleManyModelChanges = (changes: any) =>
  changes.length
...
class CircularBuffer {
  ...
  constructor(size: number, arrayType: { new (size: number): Array<any> }) {
...
class Canvas {
  ...
  readonly c: CanvasRendering2DContext;
  readonly frame: {
    domain: Range;
    range: Range;
  };
...
class StateTimeline {
  ...
  draw(events: any, earlyTime: number, lateTime: number) {
    this.canvas.clear();
...
function* izip<T>(
  a: Iterator<T> | Iterable<T>,
  b: Iterator<T> | Iterable<T>,
): Iterable<[T, T]> {
  if (!a.next) {
    a = a[Symbol.iterator]();
  }
...
let localFrameTimes: any,
  syncedFrameTimes: any,
  localTimeDeltas: any,
  syncedTimeDeltas: any;
let driftDeltaTimes: any;
let clientSmTimeline: any,
  serverSmTimeline: any,
  moduleSmTimeline: any,
...
let updateUI = () => {
  ...
  let instantFpsCanvas = new Canvas(
  ...
  if (localTimeDeltas.size) {
    ...
    let clockDriftCanvas = new Canvas(
  ...
  monitoringElement.querySelector(".label.now").textContent = (t / 1000)
    .toFixed(1);
  monitoringElement.querySelector(".label.late").textContent = (lateTime / 1000)
    .toFixed(1);
...
export function update(change: any) {
  if (enabled) {
    if (change.client) {
...
export function markDrawnModules(modulesToDraw: any) {
