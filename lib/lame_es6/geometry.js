...
export class Polygon {
  ...
  triples() {
    ...
    for (let j = 0; j < this.points.length - 1; j++) {
      const thisPt = this.points[j];
      const nextPt = this.points[j+1];
  ...
  angles() {
    function getAngle(p1, p2, p3) {
      ...
      const degs = (rads / (2 * Math.PI) * 360) % 360;
      if (isNaN(degs)) {
        ...
        if (Math.abs(u1[0] + u2[0]) < 0.00001 && Math.abs(u1[1] + u2[1]) < 0.00001) {
          ...
          console.error("got NaN angle", u1, u2);
        }
      }
      return degs;
    }
...
export function side(ax, ay, bx, by, x, y) {
  return (bx-ax)*(y-ay) - (by-ay)*(x-ax);
}
export function onLine(ax, ay, bx, by, x, y) {
  return Math.abs(side(ax, ay, bx, by, x, y)) < 0.0001 &&
              x >= Math.min(ax, bx) &&
              x <= Math.max(ax, bx) &&
              y >= Math.min(ay, by) &&
              y <= Math.max(ay, by);
}
export function onPoly(polygon, x, y) {
  for (let i=1; i < polygon.points.length; i++) {
    const a = polygon.points[i-1];
    const b = polygon.points[i];
    if (onLine(a.x, a.y, b.x, b.y, x, y)) {
      return true;
    }
  }
  return false;
...
export function isConvex(polygon) {
  ...
  for (var i=0; i < n; i++) {
    const p1 = polygon.points[i%n];
    const p2 = polygon.points[(i+1)%n];
    const p3 = polygon.points[(i+2)%n];
    const dx1 = p2.x - p1.x;
    const dy1 = p2.y - p1.y;
    const dx2 = p3.x - p2.x;
    const dy2 = p3.y - p2.y;
    const xp = dx1 * dy2 - dy1 * dx2;
    if (sign == 0) {
      sign = xp > 0 ? 1 : -1;
...
export function isInside(polygon, x, y) {
  ...
  for (let i=1; i < polygon.points.length; i++) {
    ...
    numberOfCrossings += intersect(a.x, a.y, b.x, b.y, x, y, Math.max(x, polygon.extents.x + polygon.extents.w + 1), y);
  }
...
export function intersect(ax, ay, bx, by, cx, cy, dx, dy) {
  return !!intersection(ax, ay, bx, by, cx, cy, dx, dy);
}
...
export function intersection(ax, ay, bx, by, cx, cy, dx, dy) {
...
export function intersectPolygonLine(polygon, ax, ay, bx, by) {
  for (let i=1; i < polygon.points.length; i++) {
    const m = polygon.points[i-1];
    const n = polygon.points[i];
    const isect = intersection(ax, ay, bx, by, m.x, m.y, n.x, n.y);
    if (isect) {
      return {p1: m, p2: n, u: isect.u};
    }
  }
  return null;
}
export function isInsidePolygon(testPolygon, againstPolygon) {
  // For every point in test polygon, ensure it is inside.
  for (let i=1; i < testPolygon.points.length; i++) {
    const pt = testPolygon.points[i];
    if (!isInside(againstPolygon, pt.x, pt.y)) {
      return false;
    }
  }
  return true;
}
export function intersectPolygonPolygon(testPolygon, againstPolygon) {
  for (let i=1; i < testPolygon.points.length; i++) {
    const m = testPolygon.points[i-1];
    const n = testPolygon.points[i];
    const intersection = intersectPolygonLine(againstPolygon, m.x, m.y, n.x, n.y);
    if (intersection) {
      return intersection;
    }
  }
...
export function cutPolygon(polygon, ax, ay, bx, by) {
  const leftPoints = [], rightPoints = [];
  for (let i = 0; i < polygon.points.length; ++i) {
    const p1 = polygon.points[i];
    const p2 = polygon.points[(i + 1) % polygon.points.length];

    const det1 = side(ax, ay, bx, by, p1.x, p1.y);
    const points = det1 > 0 ? leftPoints : rightPoints;
    const otherPoints = points === leftPoints ? rightPoints : leftPoints;
    ...
    if (det1 == 0) {
      ...
      const p0 = polygon.points[(i - 1 + polygon.points.length) % polygon.points.length];
      const det0 = side(ax, ay, bx, by, p0.x, p0.y);
      if (det0 == 0) {
        // Need to add it:
    ...
    const det2 = side(ax, ay, bx, by, p2.x, p2.y);
    ...
    const point = intersection(ax, ay, bx, by, p1.x, p1.y, p2.x, p2.y);
...
export function distanceToPolygon(polygon, x, y) {
  ...
  for (let i = 0; i < polygon.points.length - 1; i++) {
    const p1 = polygon.points[i];
    const p2 = polygon.points[i+1];
    const d = distanceToLine(p1.x, p1.y, p2.x, p2.y, x, y);
    minDistance = Math.min(minDistance, d);
  }
  return minDistance;
}
export function distanceToLine(x1, y1, x2, y2, xp, yp) {
  ...
  const t = ((xp - x1) * (x2 - x1) + (yp - y1) * (y2 - y1)) / l2;
  if (t <= 0) {
    return distance(xp, yp, x1, y1);
  } else if (t >= 1) {
    return distance(xp, yp, x1, y2);
  }
  const projx = x1 + t * (x2 - x1);
  const projy = y1 + t * (y2 - y1);
  return distance(xp, yp, projx, projy);
}
