...
import {dist, dot, sub, crossMag, add, scale, side, rot} from './vector2d.ts';
import {distanceToSegment, intersection, onSegment} from './line2d.ts';
...
export class Polygon {
  ...
  *iterateLatticePoints() {
    const fixupIndex = i => (i + this.points.length) % this.points.length;
    ...
    for (let y = Math.ceil(this.extents.y); y < this.extents.y + this.extents.h; y++) {
