...
export function configure({makeEmptyModule, log, time}) {
  ...
  return class ModulePlayer {
    ...
    async goToModule(module) {
      ...
      if (this.nextModule != module) {
        ...
        log(`Switching to ${this.nextModule.name} after prior preparation.`);
        module.dispose();
        return;
      }
      ...
      try {
        ...
        log.error(`Error forcing fade to black.`);
        log.error(e, {
          module: module.name,
          timestamp: time.now(),
          timestampSinceModuleStart: time.now() - module.deadline,
      ...
      log(`Finishing transition to ${module.name}.`);
