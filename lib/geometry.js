...
(function(factory) {
  ...
  const geo = {
    ...
    intersectPolygonLine(polygon, ax, ay, bx, by) {
      for (let i=1; i < polygon.points.length; i++) {
        const m = polygon.points[i-1];
        const n = polygon.points[i];
        const intersection = geo.intersection(ax, ay, bx, by, m.x, m.y, n.x, n.y);
        if (intersection) {
    ...
    isInsidePolygon(testPolygon, againstPolygon) {
    ...
    intersectPolygonPolygon(testPolygon, againstPolygon) {
      for (let i=1; i < testPolygon.points.length; i++) {
        const m = testPolygon.points[i-1];
        const n = testPolygon.points[i];
        const intersection = geo.intersectPolygonLine(againstPolygon, m.x, m.y, n.x, n.y);
        if (intersection) {
          return intersection;
        }
      }
    ...
    cutPolygon(polygon, ax, ay, bx, by) {
      const leftPoints = [], rightPoints = [];
      for (let i = 0; i < polygon.points.length; ++i) {
        const p1 = polygon.points[i];
        const p2 = polygon.points[(i + 1) % polygon.points.length];

        const det1 = geo.side(ax, ay, bx, by, p1.x, p1.y);
        ...
        if (det1 == 0) {
          ...
          const det0 = geo.side(ax, ay, bx, by, p0.x, p0.y);
          if (det0 == 0) {
            // Need to add it:
        ...
        const det2 = geo.side(ax, ay, bx, by, p2.x, p2.y);
        ...
        const point = geo.intersection(ax, ay, bx, by, p1.x, p1.y, p2.x, p2.y);
    ...
    distanceToPolygon(polygon, x, y) {
      let minDistance = Infinity;
      for (let i = 0; i < polygon.points.length - 1; i++) {
        const p1 = polygon.points[i];
        const p2 = polygon.points[i+1];
        const d = geo.distanceToLine(p1.x, p1.y, p2.x, p2.y, x, y);
        minDistance = Math.min(minDistance, d);
      }
      return minDistance;
    },
    distanceToLine(x1, y1, x2, y2, xp, yp) {
      // Reimplementation of http://stackoverflow.com/questions/849211
      const l2 = Math.pow(geo.distance(x1, y1, x2, y2), 2);
      if (l2 == 0) {
