...
(function(factory) {
  ...
  var StateMachine = function(debug, initialState) {
  ...
  StateMachine.prototype.enterNewState_ = function(state) {
    if (state === this.current_) {
      ...
      this.debug_('Self-edge in state machine!', state.name_);
      return Promise.reject('self-edge');
    }

    this.debug_('Transition from', this.current_.name_, 'to', state.name_);
    this.current_ = state;
    ...
    try {
      ...
      this.debug_('While entering', state.name_);
      this.debug_(e.message);
      this.debug_(e.stack);
    }
  ...
  var State = function(name) {
    ...
    this.transitionPromise_ = new Promise((resolve, reject) => {
  ...
  State.prototype.enter_ = function() {
    this.reject_('You must implement enter.');
  };
