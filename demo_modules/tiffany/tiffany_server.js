...
export function load(debug, wallGeometry, network) {
  /** Convert an image to grayscale. */
  function grayscale(image_u8c4) {
    const gray_u8 = new jsfeat.matrix_t(
        image_u8c4.cols, image_u8c4.rows, jsfeat.U8_t | jsfeat.C1_t);
    jsfeat.imgproc.grayscale(image_u8c4.data, image_u8c4.cols, image_u8c4.rows, gray_u8);
    return gray_u8;
  }

  /** Normalize an image. */
  function normalize(image_u8) {
    const norm_u8 = new jsfeat.matrix_t(
        image_u8.cols, image_u8.rows, jsfeat.U8_t | jsfeat.C1_t);
  ...
  function initPolygon(polygon, time, imageIndex, parentColor) {
    polygon.attrs = {
      addedAt: time,
  ...
  function markPolygonForRemoval(polygon, time) {
  ...
  function segmentPolygon(polygon) {
    ...
    for (const [p0, p1] of polygon.pairs()) {
      ...
      for (var j = 1; j < SEGMENTS_PER_EDGE; j++) {
        const frac = j / SEGMENTS_PER_EDGE;
        const newP = add(p0, scale(delta, frac));
        newP.key = false;
        newP.newPt = false;
        newPoints.push(newP);
      }
   ...
   */
  function desegmentPolygon(polygon) {
  ...
  class TiffanyServer {
        ...
        for (let edgeIndex = 0; edgeIndex < edges.length; edgeIndex++) {
          ...
          if (!(new Polygon(points).isConvex())) {
            ...
            generatedPolygons.push(desegmentPolygon(
                initPolygon(new Polygon(points), time, imageIndex, polygon.attrs.color)));
    ...
    shatterNextPoint(time) {
      ...
      const newPolygons = this.shatterPolygon(maxWeightPolygon, centroid, time);
    ...
    tick(time) {
      ...
      if (restarted || time >= this.lastUpdate + UPDATE_DELAY_MS) {
        ...
        if (polygonsToSend.length > MAX_POLYGONS) {
          polygonsToSend = polygonsToSend.slice(polygonsToSend.length - MAX_POLYGONS);
        }
